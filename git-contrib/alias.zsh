...
if ! zstyle -t ':prezto:module:git:alias' skip 'yes'; then
  ...
  alias glm='git log --topo-order --oneline --date=relative --author=`git config user.name` --pretty=format:"${_git_log_oneline_format}"'
  alias glM='git log --topo-order --oneline --stat --date=relative --author=`git config user.name` --pretty=format:"${_git_log_oneline_format}"'
